...
function Base.tanh(d::Dual)
    
    v = tanh(d.value)
    
    deriv = (1 - v^2) * d.derivative
...
X, y = random_moon(num_data) # créer un x et y et une relation entre eux 



function plot_moon(model, W, X, y) #set à ploter moon avec les couleurs 
...
function Base.isless(::Dual,::Real) #fonction isless 
    return ForwardDiff.value(Dual) < Real
end 
# 2) implémenter `max(::Real, ::Dual)`
function Base.max(::Real , ::Dual)
    return value(y) > x ? y : x
end 
# 3) implémenter `relu(::Dual)`
function Base.relu(::Dual)
    return max(0, ForwardDiff.value(Dual))
end 
